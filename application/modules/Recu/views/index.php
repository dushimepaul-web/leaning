...
<div class="dashboard-main-body">
  <div class="breadcrumb d-flex flex-wrap align-items-center justify-content-between gap-3 mb-24">
    <div>
      <h1 class="fw-semibold mb-4 h6 text-primary-light">Reçus</h1>
      <div>
        <a href="<?= base_url('Dashboard') ?>" class="text-secondary-light hover-text-primary hover-underline">Dashboard</a>
        <span class="text-secondary-light"> / Reçus</span>
      </div>
    </div>
    <div class="d-flex gap-2">
      <button type="button" class="btn btn-primary-600 d-flex align-items-center gap-6" onclick="openSidebar()">
        <span class="d-flex text-md"><i class="ri-add-line"></i></span> Nouveau reçu
      </button>
    </div>
  </div>
...
<div class="bg-white position-fixed end-0 top-0 h-100vh overflow-y-auto z-99 w-100 translate-x-full duration-300 active-translate-0" id="formSidebar" style="width:50vw;max-width:50vw;box-shadow:-4px 0 20px rgba(0,0,0,0.1);">
  <div class="px-20 py-12 border-bottom d-flex align-items-center justify-content-between gap-20">
    <h5 class="text-lg mb-0" id="formTitle">Nouveau reçu</h5>
    <button type="button" class="btn-close" onclick="closeSidebar()"></button>
  </div>
  <form id="dataForm" class="d-flex flex-column p-20">
    <div class="mb-3">
      <label class="text-sm fw-semibold text-primary-light d-inline-block mb-8">N° Reçu *</label>
      <input type="text" class="form-control" id="fNumeroRecu" placeholder="Ex: REC-2026-001">
    </div>
    ...
    <div class="mb-3">
      <label class="text-sm fw-semibold text-primary-light d-inline-block mb-8">Montant total</label>
...
async function loadData() {
  ...
  r.data.forEach((d, i) => {
    rows += `<tr>
      ...
      <td>${d.matricule || '-'}</td>
      <td><strong>${parseFloat(d.montant_total || 0).toLocaleString()}</strong></td>
      <td>${d.date_edition || '-'}</td>
      <td>
        <div class="d-flex align-items-center gap-8">
          <a class="text-success text-xl" href="${BASE_URL}Recus/imprimer/${d.uuid}" target="_blank"><i class="ri-printer-line"></i></a>
  ...
  $('#dataTable').DataTable({
    ...
    language: { search: '', searchPlaceholder: 'Rechercher...', lengthMenu: 'Lignes par page: _MENU_', info: '', zeroRecords: 'Aucun reçu trouvé', infoEmpty: '', infoFiltered: '' },
...
function openSidebar() {
  ...
  document.getElementById('formTitle').textContent = 'Nouveau reçu';
...
async function editData(uuid) {
  const r = await API.recus.get(uuid);
  if (!r.success) { Swal.fire({ icon: 'error', title: 'Erreur', text: r.message }); return; }
  editUuid = uuid;
  const d = r.data;
  document.getElementById('formTitle').textContent = 'Modifier le reçu';
...
async function deleteData(uuid) {
  const c = await Swal.fire({ title: 'Confirmation', text: 'Supprimer ce reçu ?', icon: 'warning', showCancelButton: true, confirmButtonColor: '#d33', cancelButtonText: 'Annuler', confirmButtonText: 'Oui, supprimer' });
  if (!c.isConfirmed) return;
  const r = await API.recus.delete(uuid);
...
document.getElementById('dataForm').addEventListener('submit', async function(e) {
  e.preventDefault();
  const data = { numero_recu: document.getElementById('fNumeroRecu').value, id_etudiant: document.getElementById('fEtudiant').value, montant_total: document.getElementById('fMontant').value };
  if (!data.numero_recu || !data.id_etudiant) { Swal.fire({ icon: 'warning', title: 'Validation', text: 'N° reçu et étudiant obligatoires' }); return; }
  const r = editUuid ? await API.recus.update(editUuid, data) : await API.recus.create(data);
