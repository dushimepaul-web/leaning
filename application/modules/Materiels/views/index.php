...
<div class="dashboard-main-body">
  <div class="breadcrumb d-flex flex-wrap align-items-center justify-content-between gap-3 mb-24">
    <div>
      <h1 class="fw-semibold mb-4 h6 text-primary-light">Matériels Scolaires</h1>
      <div>
        <a href="<?= base_url('Dashboard') ?>" class="text-secondary-light hover-text-primary hover-underline">Dashboard</a>
        <span class="text-secondary-light"> / Scolarité / Matériels Scolaires</span>
      </div>
    </div>
    <div class="d-flex gap-2">
      <button type="button" class="btn btn-outline-primary-600 d-flex align-items-center gap-6" onclick="initArticles()">
        <span class="d-flex text-md"><i class="ri-download-line"></i></span> Initialiser les articles
      </button>
...
async function loadData() {
  const r = await API.materiels.list();
...
async function editRecord(uuid) {
  const r = await API.materiels.get(uuid);
...
async function saveRecord() {
  const data = {
    ...
    stock_actuel: document.getElementById('fStock').value,
    stock_mini: 0,
    description: document.getElementById('fDescription').value
  };
  if (!data.libelle) { Swal.fire({ icon: 'warning', title: 'Validation', text: 'Libellé obligatoire' }); return; }
  const r = editId ? await API.materiels.update(editId, data) : await API.materiels.create(data);
...
async function deleteRecord(uuid) {
  ...
  const r = await API.materiels.delete(uuid);
  if (r.success) { Toast.fire({ icon: 'success', title: 'Produit supprimé' }); loadData(); }
  else Swal.fire({ icon: 'error', title: 'Erreur', text: r.message });
}

async function initArticles() {
  const c = await Swal.fire({ title: 'Initialiser les articles', text: 'Créer les articles pré-définis (Crayons, Taille-crayons, etc.) ?', icon: 'info', showCancelButton: true, confirmButtonText: 'Initialiser', cancelButtonText: 'Annuler' });
  if (!c.isConfirmed) return;
  const r = await API.materiels.initialiser();
...
(function() {
  var w = setInterval(function() {
    if (typeof $ !== 'undefined' && $.fn.DataTable && typeof API !== 'undefined' && API.materiels) { clearInterval(w); loadData(); }
  }, 50);
})();
</script>
