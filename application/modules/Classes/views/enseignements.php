  ...
  <div class="breadcrumb d-flex flex-wrap align-items-center justify-content-between gap-3 mb-24">
    <div>
      ...
      <div>
        ...
        <span class="text-secondary-light"> / Enseignements</span>
      </div>
    </div>
...
function openEditSidebar(data) {
  ...
  const enseignants = JSON.parse(document.getElementById('id_enseignant_data').textContent);
  const selEnseignant = enseignants.find(e => e.id_enseignant == (data.id_enseignant || ''));
  document.getElementById('id_enseignant_search').value = selEnseignant ? selEnseignant.nom + ' ' + selEnseignant.prenom : '';
  document.getElementById('id_matiere').value = data.id_matiere || '';
  const matieres = JSON.parse(document.getElementById('id_matiere_data').textContent);
  const selMatiere = matieres.find(m => m.id_matiere == (data.id_matiere || ''));
  document.getElementById('id_matiere_search').value = selMatiere ? selMatiere.libelle : '';
  document.getElementById('id_classe').value = data.id_classe || '';
  const classes = JSON.parse(document.getElementById('id_classe_data').textContent);
  const selClasse = classes.find(c => c.id_classe == (data.id_classe || ''));
  document.getElementById('id_classe_search').value = selClasse ? selClasse.libelle : '';
  document.getElementById('addSidebar').classList.add('active');
  document.getElementById('sidebarOverlay').classList.add('active');
}
...
async function loadData() {
  ...
  if (!res.success) { $('#dataBody').html('<tr><td colspan="6" class="text-center text-danger">Erreur de chargement</td></tr>'); return; }
  let rows = '';
  res.data.forEach((s, i) => {
    ...
    const enseignant = s.enseignant_prenom ? s.enseignant_prenom + ' ' + (s.enseignant_nom || '') : (s.enseignant_nom || '-');
    rows += `<tr>
      <td>${i + 1}</td>
      <td>${enseignant}</td>
...
(function() {
  var wait = setInterval(function() {
    if (typeof jQuery !== 'undefined' && $.fn && $.fn.DataTable && typeof API !== 'undefined') {
      clearInterval(wait);
      autoSetup('id_enseignant_search', 'id_enseignant', 'id_enseignant_results', JSON.parse(document.getElementById('id_enseignant_data').textContent).map(function(e) { return { id: e.id_enseignant, nom: e.nom, prenom: e.prenom }; }), function(e) { return e.nom + ' ' + e.prenom; });
      autoSetup('id_matiere_search', 'id_matiere', 'id_matiere_results', JSON.parse(document.getElementById('id_matiere_data').textContent).map(function(m) { return { id: m.id_matiere, libelle: m.libelle }; }), function(m) { return m.libelle; });
      autoSetup('id_classe_search', 'id_classe', 'id_classe_results', JSON.parse(document.getElementById('id_classe_data').textContent).map(function(c) { return { id: c.id_classe, libelle: c.libelle }; }), function(c) { return c.libelle; });
      loadData();
