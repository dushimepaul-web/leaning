...
<div class="bg-white position-fixed end-0 top-0 h-100vh overflow-y-auto z-99 w-100 translate-x-full duration-300 active-translate-0" id="addSidebar" style="width:50vw;max-width:50vw;box-shadow: -4px 0 20px rgba(0,0,0,0.1);">
  <div class="px-20 py-12 border-bottom d-flex align-items-center justify-content-between gap-20">
    <h5 class="text-lg mb-0" id="sidebarTitle">Ajouter une classe</h5>
  ...
  <form id="mainForm" class="d-flex flex-column p-20">
    ...
    <div class="row g-3">
      <div class="col-sm-12">
        ...
        <input type="text" class="form-control" id="code" placeholder="Ex: 6A, 5B">
      </div>
      <div class="col-sm-12">
        <label class="text-sm fw-semibold text-primary-light d-inline-block mb-8">Libellé *</label>
        <input type="text" class="form-control" id="libelle" placeholder="Ex: 6ème Primaire A">
      </div>
      ...
      <div class="col-sm-12">
        ...
        <input type="text" class="form-control" id="niveau" placeholder="Ex: Primaire, Secondaire">
      </div>
      <div class="col-12">
...
function openAddSidebar() {
  editingId = null;
  document.getElementById('sidebarTitle').textContent = 'Ajouter une classe';
  document.getElementById('recordId').value = '';
  document.getElementById('code').value = '';
  document.getElementById('libelle').value = '';
  document.getElementById('id_section').value = '';
  document.getElementById('id_section_search').value = '';
...
function openEditSidebar(data) {
  editingId = data.uuid;
  document.getElementById('sidebarTitle').textContent = 'Modifier la classe';
  ...
  const sections = JSON.parse(document.getElementById('id_section_data').textContent);
  const selSection = sections.find(s => s.id_section == (data.id_section || ''));
  document.getElementById('id_section_search').value = selSection ? selSection.libelle : '';
...
async function loadData() {
  ...
  res.data.forEach((c, i) => {
    ...
    rows += `<tr>
      ...
      <td>${c.libelle}</td>
      <td><span class="badge bg-info">${c.section_libelle || '-'}</span></td>
      <td>${c.niveau || '-'}</td>
      <td><span class="${statusBadge} px-24 py-4 radius-4 fw-medium text-sm">${statusText}</span></td>
      <td>
        <div class="d-flex align-items-center gap-8">
          <button type="button" class="text-primary-light text-xl" onclick="editRecord('${c.uuid}')"><i class="ri-edit-2-line"></i></button>
  ...
  $('#dataTable').DataTable({
    ...
    language: { search: '', searchPlaceholder: 'Rechercher...', lengthMenu: 'Lignes par page: _MENU_', info: '', zeroRecords: 'Aucune classe trouvée', infoEmpty: '', infoFiltered: '' },
...
document.getElementById('mainForm').addEventListener('submit', async function(e) {
  ...
  let res;
  if (editingId) res = await API.classes.update(editingId, data);
...
(function() {
  var wait = setInterval(function() {
    if (typeof jQuery !== 'undefined' && $.fn && $.fn.DataTable && typeof API !== 'undefined' && typeof autoSetup !== 'undefined') {
      clearInterval(wait);
      autoSetup('id_section_search', 'id_section', 'id_section_results', JSON.parse(document.getElementById('id_section_data').textContent).map(function(s) { return {id: s.id_section, libelle: s.libelle}; }), function(s) { return s.libelle; });
      loadData();
