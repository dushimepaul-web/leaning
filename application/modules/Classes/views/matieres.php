...
<div class="bg-white position-fixed end-0 top-0 h-100vh overflow-y-auto z-99 w-100 translate-x-full duration-300 active-translate-0" id="addSidebar" style="width:50vw;max-width:50vw;box-shadow: -4px 0 20px rgba(0,0,0,0.1);">
  <div class="px-20 py-12 border-bottom d-flex align-items-center justify-content-between gap-20">
    <h5 class="text-lg mb-0" id="sidebarTitle">Ajouter une matière</h5>
  ...
  <form id="mainForm" class="d-flex flex-column p-20">
    ...
    <div class="row g-3">
      <div class="col-sm-12">
        ...
        <input type="text" class="form-control" id="code" placeholder="Ex: MATH, FR, ANG">
      </div>
      <div class="col-sm-12">
...
function openEditSidebar(data) {
  editingId = data.uuid;
  document.getElementById('sidebarTitle').textContent = 'Modifier la matière';
...
async function loadData() {
  const res = await API.matieres.list();
  ...
  $('#dataTable').DataTable({
    ...
    language: { search: '', searchPlaceholder: 'Rechercher...', lengthMenu: 'Lignes par page: _MENU_', info: '', zeroRecords: 'Aucune matière trouvée', infoEmpty: '', infoFiltered: '' },
...
async function editRecord(id) {
  const res = await API.matieres.get(id);
...
document.getElementById('mainForm').addEventListener('submit', async function(e) {
  ...
  let res;
  if (editingId) res = await API.matieres.update(editingId, data);
