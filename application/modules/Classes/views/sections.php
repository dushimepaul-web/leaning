  ...
  <form id="mainForm" class="d-flex flex-column p-20">
    ...
    <div class="row g-3">
      <div class="col-sm-12">
        ...
        <input type="text" class="form-control" id="code" placeholder="Ex: SCI, LITT, PEDA">
      </div>
      <div class="col-sm-12">
        <label class="text-sm fw-semibold text-primary-light d-inline-block mb-8">Libellé *</label>
        <input type="text" class="form-control" id="libelle" placeholder="Ex: Scientifique, Littéraire">
      </div>
      <div class="col-12">
...
function openAddSidebar() {
  editingId = null;
  document.getElementById('sidebarTitle').textContent = 'Ajouter une section';
...
function openEditSidebar(data) {
  editingId = data.uuid;
  document.getElementById('sidebarTitle').textContent = 'Modifier la section';
...
async function loadData() {
  ...
  $('#dataTable').DataTable({
    ...
    language: { search: '', searchPlaceholder: 'Rechercher...', lengthMenu: 'Lignes par page: _MENU_', info: '', zeroRecords: 'Aucune section trouvée', infoEmpty: '', infoFiltered: '' },
...
document.getElementById('mainForm').addEventListener('submit', async function(e) {
  ...
  let res;
  if (editingId) res = await API.sections.update(editingId, data);
