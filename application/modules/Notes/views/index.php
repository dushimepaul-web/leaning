...
.classe-dropdown{position:absolute;top:calc(100% + 4px);left:0;right:0;background:#fff;border:1px solid #e5e7eb;border-radius:8px;box-shadow:0 8px 24px rgba(0,0,0,.12);max-height:220px;overflow-y:auto;z-index:1050;padding:4px;}
...
.classe-item:hover,.classe-item.active{background:#e6f5f4;color:#0d9488;}
.classe-item i{font-size:1rem;color:#0d9488;flex-shrink:0;}
.classe-empty{padding:12px;text-align:center;color:#6B7280;font-size:.85rem;}
.search-icon{position:absolute;left:14px;top:50%;transform:translateY(-50%);color:#6B7280;font-size:1rem;pointer-events:none;}
#id_classe_search, #id_matiere_search{padding-left:38px;}
      ...
      <div class="row g-2 align-items-end flex-nowrap">
        <div class="col" style="min-width:220px; position:relative;">
          ...
          <div style="position:relative;">
            ...
            <input type="text" class="form-control" id="id_classe_search" placeholder="Tapez pour chercher..." autocomplete="off">
          </div>
          <input type="hidden" id="id_classe" name="id_classe">
          <div id="id_classe_results" class="classe-dropdown d-none"></div>
        </div>
        <div class="col" style="min-width:220px; position:relative;">
          <label class="text-sm fw-semibold text-primary-light d-inline-block mb-8">Cours / Matière *</label>
          <div style="position:relative;">
            <i class="ri-search-line search-icon"></i>
            <input type="text" class="form-control" id="id_matiere_search" placeholder="Sélectionnez un cours..." autocomplete="off">
          </div>
          <input type="hidden" id="id_matiere" name="id_matiere">
          <div id="id_matiere_results" class="classe-dropdown d-none"></div>
        </div>
        ...
        <div class="col-auto d-flex gap-2 align-items-end">
          <button type="button" class="btn btn-primary-600" onclick="chargerGrilleParFiltre()"><i class="ri-search-line me-1"></i>Afficher</button>
        </div>
      </div>
    </div>
  </div>
...
function autoSetup(searchId, hiddenId, resultsId, items, labelFn, onSelect) {
  const searchInput=document.getElementById(searchId);
  const hiddenInput=document.getElementById(hiddenId);
  const resultsDiv=document.getElementById(resultsId);
  if(!searchInput)return;
  function render(list){
    if(!list.length){resultsDiv.innerHTML='<div class="classe-empty">Aucun résultat</div>';resultsDiv.classList.remove('d-none');return;}
    let h='';
    list.forEach(item=>{
      h+=`<div class="classe-item" data-id="${item.id}" data-nom="${labelFn(item)}"><i class="ri-checkbox-blank-circle-line" style="font-size:8px"></i><span>${labelFn(item)}</span></div>`;
    });
    resultsDiv.innerHTML=h;
    resultsDiv.classList.remove('d-none');
    resultsDiv.querySelectorAll('.classe-item').forEach(el=>{
  ...
  searchInput.addEventListener('input',function(){
    ...
    hiddenInput.value='';
    const filtered=items.filter(i=>labelFn(i).toLowerCase().includes(q));
    render(filtered);
  });
  ...
  document.addEventListener('click',function(e){
    if(!searchInput.contains(e.target)&&!resultsDiv.contains(e.target)){resultsDiv.classList.add('d-none');}
  });
}

async function chargerCoursPourNotes(classeId){
  const res=window.API ? await fetch(API.base_url+'api/notes/matieres_by_classe/'+classeId).then(r=>r.json()) : {success:false,data:[]};
  const matieresList=res.success?res.data.map(m=>({id:m.id_matiere,libelle:m.libelle})):[];
  document.getElementById('id_matiere').value='';
  document.getElementById('id_matiere_search').value='';
  autoSetup('id_matiere_search','id_matiere','id_matiere_results',matieresList,m=>m.libelle);
}

function chargerGrilleParFiltre(){
  const cid=document.getElementById('id_classe').value;
  const mid=document.getElementById('id_matiere').value;
  const pid=document.getElementById('id_periode').value;
  const aid=document.getElementById('id_annee').value;
  if(!cid){Swal.fire({icon:'warning',text:'Veuillez choisir une classe'});return;}
  if(!mid){Swal.fire({icon:'warning',text:'Veuillez choisir un cours'});return;}
  gCId=cid; gMid=mid;
  ACTIVE_PERIODE_ID=pid; ACTIVE_ANNEE_ID=aid;
  loadGrilleNotes();
}
...
async function loadGrilleNotes(){
  const filtresCard=document.getElementById('filtresCard');
  if(filtresCard)filtresCard.style.display='none';
  document.getElementById('grilleCard').style.display='';
...
function backToClasses(){
  gCId=null; gMid=null;
  document.getElementById('grilleCard').style.display='none';
  const filtresCard=document.getElementById('filtresCard');
