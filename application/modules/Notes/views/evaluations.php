...
async function loadData(){
  ...
  let rows='';
  r.data.forEach((e,i)=>{rows+='<tr><td>'+(i+1)+'</td><td><span class="fw-semibold">'+e.libelle+'</span></td><td>'+(e.classe||'-')+'</td><td>'+(e.matiere||'-')+'</td><td>'+(e.enseignant||'-')+'</td><td><span class="'+(typeBadges[e.type]||'')+' px-24 py-4 radius-4 fw-medium text-sm">'+(typeLabels[e.type]||e.type)+'</span></td><td>'+(e.note_max_matiere||1)+'</td><td>'+e.ponderee_sur+'</td><td>'+(e.periode||'-')+'</td><td>'+e.date_eval+'</td><td><div class="d-flex align-items-center gap-8"><button type="button" class="text-primary-light text-xl" onclick="editRecord(\''+e.uuid+'\')"><i class="ri-edit-2-line"></i></button><button type="button" class="text-danger text-xl" onclick="confirmDelete(\''+e.uuid+'\')"><i class="ri-delete-bin-6-line"></i></button></div></td></tr>';});
  $('#dataBody').html(rows);
