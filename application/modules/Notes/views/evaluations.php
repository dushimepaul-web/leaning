  ...
  <div class="card h-100">
    <div class="card-body p-0 dataTable-wrapper">
      ...
      <table class="table bordered-table mb-0 data-table" id="dataTable" style="width:100%">
        <thead>
          <tr>
            <th>#</th><th>Libellé</th><th>Classe</th><th>Matière</th><th>Enseignant</th><th>Type</th><th>Note max</th><th>Sur</th><th>Période</th><th>Date</th><th></th>
          </tr>
        </thead>
    ...
    <div class="row g-3">
      ...
      <div class="col-md-4"><label class="text-sm fw-semibold text-primary-light d-inline-block mb-8">Type</label>
        <select class="form-control form-select" id="type">
          <option value="interrogation">Interrogation</option><option value="devoir">Devoir</option><option value="ressource">Ressource</option><option value="competance">Compétence</option><option value="examen">Examen</option>
        </select>
      </div>
      ...
      <div class="col-md-6"><label class="text-sm fw-semibold text-primary-light d-inline-block mb-8">Pondérée sur</label><input type="number" class="form-control" id="ponderee_sur" step="0.1" value="20.0"></div>
...
<div class="modal fade" id="deleteModal" tabindex="-1">
  <div class="modal-dialog modal-sm modal-dialog-centered"><div class="modal-content radius-16 bg-base">
    <div class="modal-body pt-32 px-36 pb-24 text-center">
      ...
      <p class="text-sm text-secondary-light mt-8">Les notes attribuées aux élèves pour cette évaluation seront aussi effacées.<br>Cette action est irréversible.</p>
...
var typeLabels={interrogation:'Interrogation',devoir:'Devoir',ressource:'Ressource',competance:'Compétence',examen:'Examen'};
var typeBadges={interrogation:'bg-info-100 text-info-600',devoir:'bg-primary-100 text-primary-600',ressource:'bg-success-100 text-success-600',competance:'bg-warning-100 text-warning-600',examen:'bg-danger-200 text-danger-700'};
...
async function loadData(){
  ...
  let rows='';
  r.data.forEach((e,i)=>{rows+='<tr><td>'+(i+1)+'</td><td><span class="fw-semibold">'+e.libelle+'</span></td><td>'+(e.classe||'-')+'</td><td>'+(e.matiere||'-')+'</td><td>'+(e.enseignant||'-')+'</td><td><span class="'+(typeBadges[e.type]||'')+' px-24 py-4 radius-4 fw-medium text-sm">'+(typeLabels[e.type]||e.type)+'</span></td><td>'+(e.note_max_matiere||1)+'</td><td>'+e.ponderee_sur+'</td><td>'+(e.periode||'-')+'</td><td>'+e.date_eval+'</td><td><div class="btn-group"><button type="button" class="text-primary-light text-xl" data-bs-toggle="dropdown"><iconify-icon icon="tabler:dots-vertical"></iconify-icon></button><ul class="dropdown-menu dropdown-menu-lg-end border p-12"><li><button class="dropdown-item rounded text-secondary-light d-flex align-items-center gap-2 py-6" onclick="editRecord(\''+e.uuid+'\')"><i class="ri-edit-2-line"></i> Modifier</button></li><li><button class="dropdown-item rounded text-danger d-flex align-items-center gap-2 py-6" onclick="confirmDelete(\''+e.uuid+'\')"><i class="ri-delete-bin-6-line"></i> Supprimer</button></li></ul></div></td></tr>';});
  $('#dataBody').html(rows);
...
async function editRecord(id){const r=await API.evaluations.list();if(r.success){const e=r.data.find(x=>x.uuid==id);if(e){editingId=e.uuid;document.getElementById('sidebarTitle').textContent='Modifier l\'évaluation';document.getElementById('recordId').value=e.uuid;document.getElementById('libelle').value=e.libelle||'';document.getElementById('type').value=e.type||'devoir';document.getElementById('id_classe').value=e.id_classe||'';document.getElementById('id_classe_search').value=e.classe||'';document.getElementById('id_matiere').value=e.id_matiere||'';document.getElementById('id_matiere_search').value=e.matiere||'';document.getElementById('id_periode').value=e.id_periode||'';document.getElementById('date_eval').value=e.date_eval||'';document.getElementById('ponderee_sur').value=e.ponderee_sur||20;document.getElementById('addSidebar').classList.add('active');document.getElementById('sidebarOverlay').classList.add('active');}}}

document.getElementById('mainForm').addEventListener('submit',async function(e){e.preventDefault();
  var d={libelle:document.getElementById('libelle').value,id_classe:document.getElementById('id_classe').value,id_matiere:document.getElementById('id_matiere').value,id_periode:document.getElementById('id_periode').value,date_eval:document.getElementById('date_eval').value,type:document.getElementById('type').value,ponderee_sur:document.getElementById('ponderee_sur').value||20};
