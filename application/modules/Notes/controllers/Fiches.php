...
class Fiches extends MY_Controller {
    ...
    {
        $id_periode = $this->input->get('periode') ?: null;
        $id_matiere = $this->input->get('matiere') ?: null;
        $id_annee = $this->id_annee_active;

        $this->load->model('Bulletins_model', 'BulletinsModel');
        $complet = $this->BulletinsModel->get_bulletin_complet($class_id, $id_annee, $id_periode, true);

        if ($complet === null) {
            echo "<h3 style='font-family:Arial; text-align:center; margin-top:50px;'>Aucune donnée trouvée pour cette classe.</h3>";
            return;
        }
        ...
        $matiere_nom = 'TOUS LES COURS';
        if ($id_matiere && !empty($complet['matieres'])) {
            foreach ($complet['matieres'] as $m) {
                if ($m['id_matiere'] == $id_matiere) {
                    $matiere_nom = $m['libelle'];
                    $complet['matieres'] = [$m];
                    break;
                }
            }
        ...
        $periode_nom = '';
        if ($id_periode && $id_periode !== 'all' && !empty($complet['periodes'])) {
            foreach ($complet['periodes'] as $p) {
                if ($p['id_periode'] == $id_periode) {
                    $periode_nom = $p['libelle'];
                    break;
                }
            }
        } else {
