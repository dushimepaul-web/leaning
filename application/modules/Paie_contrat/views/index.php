...
<div class="dashboard-main-body">
  <div class="breadcrumb d-flex flex-wrap align-items-center justify-content-between gap-3 mb-24">
    <div>
      <h1 class="fw-semibold mb-4 h6 text-primary-light">Contrats de paie</h1>
      <div>
        <a href="<?= base_url('Dashboard') ?>" class="text-secondary-light hover-text-primary hover-underline">Dashboard</a>
        <span class="text-secondary-light"> / Contrats de paie</span>
      </div>
    </div>
    <div class="d-flex gap-2">
      <button type="button" class="btn btn-primary-600 d-flex align-items-center gap-6" onclick="openSidebar()">
        <span class="d-flex text-md"><i class="ri-add-line"></i></span> Nouveau contrat
      </button>
    </div>
  </div>
...
<div class="bg-white position-fixed end-0 top-0 h-100vh overflow-y-auto z-99 w-100 translate-x-full duration-300 active-translate-0" id="formSidebar" style="width:50vw;max-width:50vw;box-shadow:-4px 0 20px rgba(0,0,0,0.1);">
  <div class="px-20 py-12 border-bottom d-flex align-items-center justify-content-between gap-20">
    <h5 class="text-lg mb-0" id="formTitle">Nouveau contrat</h5>
  ...
  <form id="dataForm" class="d-flex flex-column p-20">
    <div class="mb-3">
      ...
      <select class="form-control form-select" id="fEmploye">
    ...
    <div class="mb-3">
      ...
      <input type="number" class="form-control" id="fSalaire" step="0.01" placeholder="0.00">
    </div>
    <div class="mb-3">
      <label class="text-sm fw-semibold text-primary-light d-inline-block mb-8">Date début *</label>
      <input type="date" class="form-control" id="fDebut">
    </div>
    <div class="mb-3">
...
<script id="employes_data" type="application/json"><?= json_encode($employes) ?></script>
<script src="<?= base_url() ?>assets/js/api.js?v=<?= filemtime(FCPATH.'assets/js/api.js') ?>"></script>
<script>
const Toast = Swal.mixin({ toast: true, position: 'top-end', showConfirmButton: false, timer: 3000, timerProgressBar: true });
let editUuid = null;
let employesData = [];
try { const el = document.getElementById('employes_data'); if (el) employesData = JSON.parse(el.textContent); } catch(e) {}

async function loadData() {
  const r = await API.paie_contrats.list();
  if (!r.success) { $('#dataBody').html('<tr><td colspan="9" class="text-center text-danger">Erreur</td></tr>'); return; }
  let rows = '';
  r.data.forEach((d, i) => {
    const statutClass = d.statut === 'actif' ? 'success' : d.statut === 'termine' ? 'secondary' : 'warning';
    rows += `<tr>
      <td>${i + 1}</td>
      <td class="fw-semibold">${d.nom_complet}</td>
      <td>${d.matricule || '-'}</td>
      <td>${d.type_employe}</td>
      <td><strong>${parseFloat(d.salaire_base || 0).toLocaleString()}</strong></td>
      <td>${d.date_debut}</td>
      <td>${d.date_fin || '-'}</td>
...
function openSidebar() {
  ...
  document.getElementById('formTitle').textContent = 'Nouveau contrat';
...
async function editData(uuid) {
  const r = await API.paie_contrats.get(uuid);
  if (!r.success) { Swal.fire({ icon: 'error', title: 'Erreur', text: r.message }); return; }
  editUuid = uuid;
  const d = r.data;
  document.getElementById('formTitle').textContent = 'Modifier le contrat';
  document.getElementById('fEmploye').value = d.id_employe || '';
  document.getElementById('fType').value = d.type_employe || 'personnel';
  document.getElementById('fSalaire').value = d.salaire_base || '';
  document.getElementById('fDebut').value = d.date_debut || '';
  document.getElementById('fFin').value = d.date_fin || '';
  document.getElementById('formOverlay').classList.remove('visibility-hidden', 'opacity-0');
  document.getElementById('formSidebar').classList.remove('translate-x-full');
}

async function deleteData(uuid) {
  const c = await Swal.fire({ title: 'Confirmation', text: 'Supprimer ce contrat ?', icon: 'warning', showCancelButton: true, confirmButtonColor: '#d33', cancelButtonText: 'Annuler', confirmButtonText: 'Oui, supprimer' });
  if (!c.isConfirmed) return;
  const r = await API.paie_contrats.delete(uuid);
...
document.getElementById('dataForm').addEventListener('submit', async function(e) {
  e.preventDefault();
  const data = { id_employe: document.getElementById('fEmploye').value, type_employe: document.getElementById('fType').value, salaire_base: document.getElementById('fSalaire').value, date_debut: document.getElementById('fDebut').value, date_fin: document.getElementById('fFin').value };
  if (!data.id_employe || !data.salaire_base) { Swal.fire({ icon: 'warning', title: 'Validation', text: 'Employé et salaire obligatoires' }); return; }
  const r = editUuid ? await API.paie_contrats.update(editUuid, data) : await API.paie_contrats.create(data);
...
(function() {
  var _f = function() {
    ...
    loadData();
    const sel = document.getElementById('fEmploye');
    employesData.forEach(function(e) { var o = document.createElement('option'); o.value = e.id_employe; o.textContent = e.nom_complet + ' (' + (e.matricule || '') + ')'; sel.appendChild(o); });
  };
  _f();
})();
</script>
