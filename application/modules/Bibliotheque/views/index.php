...
<div class="dashboard-main-body">
  <div class="breadcrumb d-flex flex-wrap align-items-center justify-content-between gap-3 mb-24">
    <div>
      <h1 class="fw-semibold mb-4 h6 text-primary-light">Bibliothèque</h1>
      <div>
        <a href="<?= base_url('Dashboard') ?>" class="text-secondary-light hover-text-primary hover-underline">Dashboard</a>
        <span class="text-secondary-light"> / Bibliothèque</span>
      </div>
    </div>
...
<div class="bg-white position-fixed end-0 top-0 h-100vh overflow-y-auto z-99 w-100 translate-x-full duration-300 active-translate-0" id="addSidebar" style="width:50vw;max-width:50vw;box-shadow: -4px 0 20px rgba(0,0,0,0.1);">
  <div class="px-20 py-12 border-bottom d-flex align-items-center justify-content-between gap-20">
    <h5 class="text-lg mb-0" id="sidebarTitle">Ajouter un livre</h5>
  ...
  <form id="mainForm" class="d-flex flex-column p-20">
    ...
    <div class="row g-3">
      <div class="col-md-6">
        <label class="text-sm fw-semibold text-primary-light d-inline-block mb-8">Titre *</label>
        <input type="text" class="form-control" id="titre" required placeholder="Titre du livre">
      </div>
      <div class="col-md-6">
        <label class="text-sm fw-semibold text-primary-light d-inline-block mb-8">Auteur</label>
        <input type="text" class="form-control" id="auteur" placeholder="Nom de l'auteur">
      </div>
      <div class="col-md-6">
        <label class="text-sm fw-semibold text-primary-light d-inline-block mb-8">ISBN</label>
        <input type="text" class="form-control" id="isbn" placeholder="978-xxx-xxx-xxx-x">
      </div>
      <div class="col-md-6">
        <label class="text-sm fw-semibold text-primary-light d-inline-block mb-8">Éditeur</label>
        <input type="text" class="form-control" id="editeur" placeholder="Maison d'édition">
      </div>
      <div class="col-md-6">
        <label class="text-sm fw-semibold text-primary-light d-inline-block mb-8">Année publication</label>
        <input type="number" class="form-control" id="annee_publication" placeholder="2024">
      </div>
      <div class="col-md-6">
        <label class="text-sm fw-semibold text-primary-light d-inline-block mb-8">Emplacement</label>
        <input type="text" class="form-control" id="emplacement" placeholder="Ex: Rayon A-1">
      </div>
      <div class="col-md-4">
        <label class="text-sm fw-semibold text-primary-light d-inline-block mb-8">Quantité totale</label>
        <input type="number" class="form-control" id="quantite_totale" value="1" min="1">
      </div>
      <div class="col-md-4">
        <label class="text-sm fw-semibold text-primary-light d-inline-block mb-8">Prix unitaire</label>
        <input type="number" class="form-control" id="prix_unitaire" step="0.01" value="0" placeholder="0.00">
      </div>
      <div class="col-12">
...
async function loadData() {
  const res = await API.bibliotheque.list();
  if (!res.success) { $('#dataBody').html('<tr><td colspan="10" class="text-center text-danger">Erreur</td></tr>'); return; }
  let rows = '';
  res.data.forEach((l, i) => {
    const stockBadge = l.quantite_disponible > 0 ? 'bg-success-100 text-success-600' : 'bg-danger-100 text-danger-600';
    rows += `<tr>
      <td>${i + 1}</td>
      <td><span class="fw-semibold">${l.titre}</span></td>
      <td>${l.auteur || '-'}</td>
      <td>${l.isbn || '-'}</td>
      <td>${l.editeur || '-'}</td>
      <td>${l.annee_publication || '-'}</td>
      <td>${l.quantite_totale || 0}</td>
      <td><span class="${stockBadge} px-24 py-4 radius-4 fw-medium text-sm">${l.quantite_disponible || 0}</span></td>
      <td>${parseFloat(l.prix_unitaire || 0).toLocaleString()}</td>
      <td>
        <div class="btn-group">
          ...
          <ul class="dropdown-menu dropdown-menu-lg-end border p-12">
            <li><button class="dropdown-item rounded text-secondary-light d-flex align-items-center gap-2 py-6" onclick="editRecord('${l.uuid}')"><i class="ri-edit-2-line"></i> Modifier</button></li>
            <li><button class="dropdown-item rounded text-secondary-light d-flex align-items-center gap-2 py-6" onclick="confirmDelete('${l.uuid}')"><i class="ri-delete-bin-6-line"></i> Supprimer</button></li>
          </ul>
        </div>
      </td>
    </tr>`;
  ...
  $('#dataTable').DataTable({
    ...
    language: { search: '', searchPlaceholder: 'Rechercher...', lengthMenu: 'Lignes par page: _MENU_', info: '', zeroRecords: 'Aucun livre trouvé', infoEmpty: '', infoFiltered: '' },
...
function openAddSidebar() {
  editingId = null;
  document.getElementById('sidebarTitle').textContent = 'Ajouter un livre';
  document.getElementById('mainForm').reset();
  document.getElementById('recordId').value = '';
  document.getElementById('quantite_totale').value = '1';
...
function openEditSidebar(data) {
  editingId = data.uuid;
  document.getElementById('sidebarTitle').textContent = 'Modifier le livre';
  document.getElementById('recordId').value = data.uuid;
  document.getElementById('titre').value = data.titre || '';
  document.getElementById('auteur').value = data.auteur || '';
  document.getElementById('isbn').value = data.isbn || '';
  document.getElementById('editeur').value = data.editeur || '';
  document.getElementById('annee_publication').value = data.annee_publication || '';
  document.getElementById('emplacement').value = data.emplacement || '';
  document.getElementById('quantite_totale').value = data.quantite_totale || 1;
  document.getElementById('prix_unitaire').value = data.prix_unitaire || 0;
  document.getElementById('addSidebar').classList.add('active');
  document.getElementById('sidebarOverlay').classList.add('active');
}
...
async function editRecord(id) {
  const res = await API.bibliotheque.list();
  if (res.success) {
    const item = res.data.find(l => l.uuid == id);
...
document.getElementById('mainForm').addEventListener('submit', async function(e) {
  ...
  const data = {
    ...
    quantite_totale: document.getElementById('quantite_totale').value || 1,
    prix_unitaire: document.getElementById('prix_unitaire').value || 0
  };
  if (!data.titre) { Swal.fire({ icon: 'warning', title: 'Validation', text: 'Titre obligatoire' }); return; }
  let r;
  if (editingId) {
    r = await API.bibliotheque.update(editingId, data);
  } else {
    data.quantite_disponible = data.quantite_totale;
    r = await API.bibliotheque.create(data);
  }
  if (r.success) {
    closeSidebar();
    Toast.fire({ icon: 'success', title: editingId ? 'Livre modifié' : 'Livre ajouté' });
    loadData();
...
document.getElementById('confirmDeleteBtn').addEventListener('click', async function() {
  if (!deleteId) return;
  const r = await API.bibliotheque.delete(deleteId);
  if (r.success) {
    bootstrap.Modal.getInstance(document.getElementById('deleteModal')).hide();
    Toast.fire({ icon: 'success', title: 'Livre supprimé' });
    loadData();
  } else { Swal.fire({ icon: 'error', title: 'Erreur', text: r.message }); }
  deleteId = null;
});
...
function exportCSV() {
  ...
  let csv = '\uFEFF';
  const headers = ['#', 'Titre', 'Auteur', 'ISBN', 'Éditeur', 'Année', 'Total', 'Disponible', 'Prix'];
  csv += headers.join(',') + '\n';
  ...
  link.download = 'bibliotheque.csv';
  link.click();
}
