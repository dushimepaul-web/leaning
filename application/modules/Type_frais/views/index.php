  ...
  <div class="breadcrumb d-flex flex-wrap align-items-center justify-content-between gap-3 mb-24">
    ...
    <div class="d-flex gap-2">
      <button type="button" class="btn btn-primary-600 d-flex align-items-center gap-6" onclick="openSidebar()">
        <span class="d-flex text-md"><i class="ri-add-line"></i></span> Nouveau type
      </button>
    </div>
  </div>
    ...
    <div class="card-body p-0 dataTable-wrapper">
      <div class="d-flex align-items-center justify-content-between flex-wrap gap-16 px-20 py-12 border-bottom border-neutral-200">
        <form class="navbar-search dt-search m-0">
          <input type="text" id="dtSearch" class="dt-input bg-transparent radius-4" aria-controls="typesTable" name="search" placeholder="Rechercher...">
          <iconify-icon icon="ion:search-outline" class="icon"></iconify-icon>
        </form>
        <div class="d-flex align-items-center gap-8 text-secondary-light">
          <span>Lignes par page :</span>
          <div class="dt-length"><select id="dtLength" name="typesTable_length" class="dt-input form-control form-select">
      ...
      <table class="table bordered-table mb-0 data-table" id="typesTable" style="width:100%">
        <thead>
          <tr>
            <th>#</th>
            <th>Code</th>
...
<div class="bg-white position-fixed end-0 top-0 h-100vh overflow-y-auto z-99 w-100 translate-x-full duration-300 active-translate-0" id="formSidebar" style="width:50vw;max-width:50vw;box-shadow:-4px 0 20px rgba(0,0,0,0.1);">
  <div class="px-20 py-12 border-bottom d-flex align-items-center justify-content-between gap-20">
    <h5 class="text-lg mb-0" id="formTitle">Ajouter un type de frais</h5>
...
async function loadData() {
  const r = await API.types_frais.list();
  if (!r.success) { $('#typesBody').html('<tr><td colspan="5" class="text-center text-danger">Erreur</td></tr>'); return; }
  let rows = '';
  r.data.forEach((d, i) => {
    rows += `<tr>
      <td>${i + 1}</td>
      ...
      <td>
        <div class="d-flex align-items-center gap-8">
          <button type="button" class="text-primary-light text-xl" onclick="editData('${d.uuid}')"><i class="ri-edit-2-line"></i></button>
          <button type="button" class="text-danger text-xl" onclick="deleteData('${d.uuid}')"><i class="ri-delete-bin-6-line"></i></button>
        </div>
      </td>
    </tr>`;
  });
  ...
  $('#typesTable').DataTable({
    ...
    language: { search: '', searchPlaceholder: 'Rechercher...', lengthMenu: 'Lignes par page: _MENU_', info: '', zeroRecords: 'Aucun type trouvé', infoEmpty: '', infoFiltered: '' },
...
function openSidebar() {
  ...
  document.getElementById('formTitle').textContent = 'Ajouter un type de frais';
...
async function editData(uuid) {
  const r = await API.types_frais.get(uuid);
  if (!r.success) { Swal.fire({ icon: 'error', title: 'Erreur', text: r.message }); return; }
  editUuid = uuid;
  document.getElementById('formTitle').textContent = 'Modifier le type de frais';
  document.getElementById('typeCode').value = r.data.code || '';
  document.getElementById('typeLibelle').value = r.data.libelle || '';
  document.getElementById('typeDescription').value = r.data.description || '';
  document.getElementById('formOverlay').classList.remove('visibility-hidden', 'opacity-0');
  document.getElementById('formSidebar').classList.remove('translate-x-full');
}

async function deleteData(uuid) {
  const c = await Swal.fire({ title: 'Confirmation', text: 'Supprimer ce type de frais ?', icon: 'warning', showCancelButton: true, confirmButtonColor: '#d33', cancelButtonText: 'Annuler', confirmButtonText: 'Oui, supprimer' });
...
document.getElementById('typeForm').addEventListener('submit', async function(e) {
  ...
  const r = editUuid ? await API.types_frais.update(editUuid, data) : await API.types_frais.create(data);
...
document.getElementById('dtSearch').addEventListener('keyup', function() { if ($.fn.DataTable.isDataTable('#typesTable')) $('#typesTable').DataTable().search(this.value).draw(); });
document.getElementById('dtLength').addEventListener('change', function() { if ($.fn.DataTable.isDataTable('#typesTable')) $('#typesTable').DataTable().page.len(+this.value).draw(); });
