...
<div class="dashboard-main-body">
  <div class="breadcrumb d-flex flex-wrap align-items-center justify-content-between gap-3 mb-24">
    ...
    <a href="<?= base_url('Enseignants/add') ?>" class="btn btn-primary-600 d-flex align-items-center gap-6">
            ...
            <div class="dropdown">
              ...
              <div class="dropdown-menu border bg-base shadow dropdown-menu-lg p-0">
                ...
                <form class="p-16 d-grid grid-cols-2 gap-16">
                  ...
                  <div>
                    <label class="text-sm fw-semibold text-primary-light d-inline-block mb-8">Contract</label>
                    <select id="filterContract" class="form-control form-select">
                      <option value="">All</option>
                      <option value="Contractuel">Contractual</option>
                      <option value="Horaire">Hourly</option>
                    </select>
                  </div>
...
async function loadData() {
  ...
  res.data.forEach((e, i) => {
    const statusBadge = e.actif == 1 ? 'bg-success-100 text-success-600' : 'bg-danger-100 text-danger-600';
    const statusText = e.actif == 1 ? 'Active' : 'Inactive';
    rows += `<tr>
      <td>${i + 1}</td>
      <td><span class="text-primary-600">${e.matricule || '-'}</span></td>
      <td>
        <div class="d-flex align-items-center">
          <img src="${BASE_URL}assets/images/thumbs/avatar-img1.png" alt="" class="flex-shrink-0 me-12 radius-8" style="width:40px;height:40px;object-fit:cover;">
  ...
  $('#dataTable').DataTable({
    pageLength: +$('#dtLength').val(),
    scrollX: true,
    lengthMenu: [[5, 10, 25, 50, 100], [5, 10, 25, 50, 100]],
    language: { search: '', searchPlaceholder: 'Search...', lengthMenu: 'Rows per page: _MENU_', info: '', zeroRecords: 'No teachers found', infoEmpty: '', infoFiltered: '' },
...
function resetFilters() { $('#filterGender, #filterStatus, #filterContract').val(''); loadData(); }

async function deleteRecord(id) {
  const result = await Swal.fire({ title: 'Delete Teacher?', text: 'This action cannot be undone!', icon: 'warning', showCancelButton: true, confirmButtonText: 'Yes, delete', cancelButtonText: 'Cancel', customClass: { confirmButton: 'swal2-styled--danger' } });
  if (!result.isConfirmed) return;
  const res = await API.enseignants.delete(id);
  if (res.success) { Toast.fire({ icon: 'success', title: res.message || 'Teacher deleted' }); loadData(); }
...
function exportCSV() {
  ...
  let csv = '\uFEFF';
  const headers = ['#', 'ID', 'Name', 'Email', 'Phone', 'Join Date', 'Status'];
  csv += headers.join(',') + '\n';
