...
<div class="modal fade" id="addModal" tabindex="-1">
  <div class="modal-dialog">
    <div class="modal-content">
      <div class="modal-header"><h5 class="modal-title">Ajouter un menu</h5><button type="button" class="btn-close" data-bs-dismiss="modal"></button></div>
      <div class="modal-body">
        <form id="addForm">
          ...
          <div class="mb-3"><label class="form-label">Icône</label><input type="text" name="icon" class="form-control"></div>
          <div class="mb-3"><label class="form-label">Ordre</label><input type="number" name="ordre" class="form-control" value="0"></div>
          <div class="mb-3"><label class="form-label">Menu parent</label>
            <select name="parent_id" class="form-select"><option value="">Aucun (menu racine)</option></select>
          </div>
        </form>
      </div>
      <div class="modal-footer">
        <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Fermer</button>
        <button type="button" class="btn btn-primary-600" onclick="saveMenu()">Enregistrer</button>
      </div>
    </div>
  </div>
</div>
...
async function loadTable() {
  ...
  (res.data || []).forEach((m, i) => {
    html += `<tr>
      ...
      <td>${m.ordre ?? 0}</td>
      <td>
        <button class="btn btn-sm btn-danger" onclick="confirmDelete('${m.uuid}')"><i class="ri-delete-bin-line"></i></button>
      </td>
    </tr>`;
  });
...
async function saveMenu() {
  ...
  const res = await API.menus.create(data);
...
async function confirmDelete(uuid) {
  const c = await Swal.fire({ title: 'Confirmer', text: 'Supprimer ce menu ?', icon: 'warning', showCancelButton: true });
  if (c.isConfirmed) { const r = await API.menus.delete(uuid); if (r.success) loadTable(); else Swal.fire('Erreur', r.message, 'error'); }
}
