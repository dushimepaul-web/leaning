...
<head>
  ...
  <title>R̩initialiser le mot de passe - <?= $this->Model->get_setting('nom_ecole', 'VIP School') ?></title>
...
<body>
  ...
  <div class="d-lg-flex bg-white" style="min-height:100vh;">
    <div class="w-50 d-lg-flex d-none overflow-hidden">
      ...
      <img src="<?= base_url($login_img) ?>" alt="R̩initialisation" class="w-100 h-100 object-fit-cover" onerror="this.style.display='none';this.parentElement.style.background='linear-gradient(135deg,#25A194 0%,#1C7F73 100%)'">
    </div>
    <div class="lg-w-50 px-24 py-32 d-flex justify-content-center align-items-center">
      <div class="max-w-540-px mx-auto w-100">
        ...
        <div class="mt-32 mb-32">
          <h1 class="h6 fw-bold text-primary-light mb-8">R̩initialiser le mot de passe</h1>
        ...
        <form action="<?= base_url('Admin/do_reset_password') ?>" method="POST" class="d-flex flex-column gap-32">
          ...
          <div>
            <button type="submit" class="btn btn-primary-600 text-sm btn-sm px-12 py-16 w-100 radius-8">R̩initialiser</button>
          </div>
        </form>
