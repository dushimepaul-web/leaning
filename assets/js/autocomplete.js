function autoRender(items, query, labelFn) {
  ...
  if (!matches.length) return '<div class="list-group-item text-secondary-light text-center py-2 text-sm">Aucun r&eacute;sultat</div>';
...
function autoSetup(inputId, hiddenId, resultsId, items, labelFn, onSelect) {
  ...
  input.addEventListener('keydown', function(e) {
    if (e.key === 'ArrowDown' || e.key === 'ArrowUp') {
      ...
      var idx = Array.from(btns).indexOf(document.activeElement);
